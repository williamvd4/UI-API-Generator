    ...
    <div className="flex-1 overflow-auto rounded border border-gray-700 bg-gray-900 p-3 text-xs text-gray-300">
      ...
      <div className="mb-3 grid grid-cols-2 gap-2">
        <div>
          ...
          <pre className="max-h-60 overflow-auto rounded bg-gray-800 p-2 whitespace-pre-wrap break-words">{JSON.stringify(request.headers, null, 2)}</pre>
        </div>
        <div>
          <div className="mb-1 text-gray-400">Params</div>
          <pre className="max-h-60 overflow-auto rounded bg-gray-800 p-2 whitespace-pre-wrap break-words">{JSON.stringify(params, null, 2)}</pre>
        </div>
      </div>
      <div className="text-gray-400">JSON</div>
      <pre className="max-h-80 overflow-auto rounded bg-gray-800 p-2 text-[11px] whitespace-pre-wrap break-words">{JSON.stringify(request.json, null, 2)}</pre>
    </div>
  );
}
